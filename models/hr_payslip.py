...
class HrPayslip(models.Model):
    ...
    id_include_jkk_jkm = fields.Boolean(default=True)  # jkk jkm jht jp
    id_include_bpjs_kesehatan = fields.Boolean(default=True)  # bpjs kesehatan + its deduction
    id_include_pkp_ptkp = fields.Boolean(
        compute="_compute_id_include_pkp_ptkp", store=True, readonly=False)  # include the PTKP/PKP computation

    def _id_get_historical_categorical_total(self, codes):
    ...
    def _id_get_pph21_amount(self, amount):
        """ Find the right percentage to apply depending on the GROSS amount of the payslip"""
        category_type = KODE_PTKP_MAPPING[self.employee_id.id_kode_ptkp]

        ranges = self._rule_parameter('id_pph21_percentage_' + category_type)
        for line in ranges:
            if amount < line[1]:
                return line[2]

    def _id_get_worked_days_rate(self):
    ...
    def _id_get_end_total_pph_amount(self):
        """ Getting the accumulated PPH21 amount over the course of a year (from start of year/contract)"""
        return self._id_get_historical_categorical_total(['PPH21'])

    def _id_get_gross_accumulated(self):
        # gross_lines = self._id_get_historical_categorical_total_lines(['GROSS'])
        date_start = date(self.date_to.year, 1, 1)
        payslips = self.env['hr.payslip'].search([
            ('date_to', '>', date_start),
            ('date_to', '<', self.date_to),
        ...
        sum_gross = 0
        threshold = self._rule_parameter('id_biaya_jabatan_salary_threshold')
        percent = self._rule_parameter('id_biaya_jabatan_percent')
